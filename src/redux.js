...
function createStore(reducer, preloadedState, enhancer) {
  ...
  function dispatch(action) {
    //dispatch一定是一个对象，typeof action 是不是object 与null
  ...
  //dispatch是  isDispatching会变成true，为true时，应为建立store时，会将reducer函数指向currentRender
  /*
  当 isDiapatching 为true的时候，会调用render函数即currentRender,传入 currentState 即store创建时传入的preloadedState,
  和action   返回值是一个state。即currentStatue     而在
  */
    if (isDispatching) {
      throw new Error(
        process.env.NODE_ENV === "production"
